...
public:
	...
	ENDPOINT(API_M_GET, "/Title/query", queryTitle, API_HANDLER_AUTH_PARAME, QUERIES(QueryParams, queryParams)) {
		// 解析查询参数
	...
	// 3.2 定义新增接口处理
	ENDPOINT(API_M_POST, "/Title/add", addTitle, BODY_DTO(TitleDTO::Wrapper, dto)) {
		// 响应结果
	...
	// 3.2 定义修改接口处理
	ENDPOINT(API_M_PUT, "/Title/modify", modifyTitle, BODY_DTO(TitleDTO::Wrapper, dto)) {
		// 响应结果
	...
	// 3.2 定义删除接口处理
	ENDPOINT(API_M_DEL, "/Title/delete", removeTitle, BODY_DTO(TitleDTO::Wrapper, dto)) {
		// 响应结果
