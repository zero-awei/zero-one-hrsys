...
#include "ApiHelper.h"
#include "domain/query/test/TestQuery.h"
#include "domain/vo/test/TestVO.h"
...
public: // 定义接口
	...
	ENDPOINT_INFO(queryTest) {
		...
		// 定义响应参数类型
		API_DEF_ADD_RSP_JSON_WRAPPER(TestPageJsonVO);
		// 定义分页查询参数描述
	...
	ENDPOINT(API_M_GET,"/test", queryTest, QUERIES(QueryParams, qps)) {
		// 解析查询参数（解析成领域模型对象）
		API_HANDLER_QUERY_PARAM(query, TestQuery, qps);
		// 响应结果
		API_HANDLER_RESP_VO(execQueryTest(query));
	}
private: // 定义接口执行函数
	// 5 定义接口的执行函数
	TestPageJsonVO::Wrapper execQueryTest(const TestQuery::Wrapper& query);
};
